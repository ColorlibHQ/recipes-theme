...
function recipes_post_cats() {
	...
	if ( $cats ) {
		...
		$categories .= '<ul class="nav"><li><span><i class="fa fm fa-th-list"></i>' . esc_html( 'Catagory :', 'recipes' ) . '</span></li>';
...
// Blog categories
function recipes_featured_post_cat(){
...
// social media
if ( ! function_exists( 'recipes_social' ) ) {
	function recipes_social( $args = array() ) {
		...
		$url = recipes_opt( 'recipes_social_url' );
...
// header cart count
function recipes_cart_count( $class = '' ) {

	?>
	<div class="header-wrapicon2 <?php echo esc_attr( $class ); ?>">
		<img src="<?php echo esc_url( RECIPES_DIR_ASSETS_URI ); ?>img/icons/icon-header-02.png" class="header-icon1 js-show-header-dropdown" alt="<?php esc_attr_e( 'ICON', 'recipes' ); ?>">
...
function recipes_contact7_form_content( $template, $prop ) {
...
add_filter( 'wpcf7_default_template', 'recipes_contact7_form_content', 10, 2 );