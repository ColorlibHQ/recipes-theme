...
function recipes_pagination( $args = array() ) {
    
    $defaults = array(
        'range'           => 4,
        'custom_query'    => FALSE,
        'previous_string' => esc_html__( '&laquo;', 'recipes' ),
        'next_string'     => esc_html__( '&raquo;', 'recipes' ),
        'before_output'   => '<div class="recipes-pagination-area col-12"><nav><ul class="pagination">',
        'after_output'    => '</ul></nav></div>'
    );
    
    $args = wp_parse_args( 
        $args, 
        apply_filters( 'recipes_pagination_defaults', $defaults )
    );
